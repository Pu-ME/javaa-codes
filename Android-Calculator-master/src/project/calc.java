...
public class calc extends Application   {
	...
	public static double Evaluate_Postfix (GenericStack<String> post)
	{
		...
		while(!post.isEmpty())
		{
			if(!(post.peek().equals("^"))&&!(post.peek().equals("+"))&&!(post.peek().equals("-"))&& !(post.peek().equals("*"))&&!(post.peek().equals("/"))&&!(post.peek().equals("%")))
			{
				if(post.peek().contains("~"))
				{
					String tempr=post.pop();
					tempr=tempr.replace('~','-');
					post.push(tempr);
				}
				resultStack.push(Double.valueOf(post.pop()));
			...
			else if((post.peek().equals("^")||post.peek().equals("%"))||(post.peek().equals("+")||post.peek().equals("-")||post.peek().equals("*")||post.peek().equals("/")||post.peek().equals("ط¯â‚¬")))
			{
				...
				Operand1=resultStack.pop();
				 key=operation(Operator,Operand1,Operand2);
	...
	public static String conversion(char[]g)
	{
		...
		//char []g={'6','/','2','-','3','+','4','*','2'};
		GenericStack<String> h ;
		GenericStack<String> j=new GenericStack<>() ;
		h=infix_to_postfix(g);
		for(int i=h.getSize()-1;i>-1;i--)
		{
			j.push(h.getIndex(i));
		}
			String result=	String.valueOf((Evaluate_Postfix(j)));
		return result;
	}
	@Override
	//the start method 
